...
팀 내부에서 브라우저로 쓰기 위한 Cloudflare Workers Static Assets 버전도 들어 있습니다. Cloudflare의 Git 빌드 화면에서 deploy command가 필수인 경우 이 방식이 맞습니다.
...
Workers 무료 플랜의 외부 subrequest 제한을 피하기 위해 웹앱은 분석 요청을 회사 묶음 단위로 자동 분할합니다. 2026년처럼 OpenDART 재무제표 API가 비어 있어 XBRL fallback까지 타는 경우도 감안해, 브라우저가 여러 번의 작은 `/api/analyze` 요청으로 나눠 실행하고 마지막에 화면에서 순위를 다시 계산합니다.
