...
팀 내부에서 브라우저로 쓰기 위한 Vercel 버전이 들어 있습니다. 정적 UI는 `public/`에서 서빙하고, `/api/*` 요청은 Vercel Function이 기존 분석 로직으로 넘깁니다.

- 정적 UI: `public/`
- Vercel API 엔트리포인트: `api/[...path].js`
- 공통 API 라우트: `functions/api/`
...
Vercel 대시보드에서는 GitHub 저장소를 가져온 뒤 환경변수에 `DART_API_KEY`를 등록하세요. GitHub 연동 배포를 쓸 때는 Framework Preset을 `Other`로 두고, Build Command는 `npm run check`, Root Directory는 `/`로 두면 됩니다. 정적 파일은 Vercel의 기본 `public/` 규칙으로 배포됩니다. `vercel.json`에도 `buildCommand`가 들어 있어 대시보드 설정을 비워도 같은 검사를 실행합니다.

웹앱은 분석을 회사 1개 단위의 작은 `/api/analyze` 요청으로 쪼개 실행합니다. Vercel에서는 Cloudflare Worker의 subrequest 제한을 피할 수 있고, 2026년처럼 OpenDART 재무제표 API가 비어 있어 XBRL fallback까지 타는 경우에도 한 요청이 과도하게 커지지 않습니다.

Vercel 빌드는 Python 설치가 필요 없습니다. Qt GUI가 필요할 때만 로컬에서 `requirements-desktop.txt`를 설치하면 됩니다.

웹앱은 로컬 `out/` 폴더를 만들지 않습니다. 분석 결과와 공시 목록은 화면에서 `CSV`로 내려받고, PDF는 행별 `PDF` 버튼으로 DART 원문을 다운로드합니다.
...
Cloudflare용 설정은 `wrangler.toml`, `src/index.js`, `npm run deploy:cloudflare`로 남겨두었습니다. 다만 OpenDART가 Cloudflare Worker 출구 요청을 `error1.html`로 튕기는 경우 Vercel 배포를 우선 사용하세요.
