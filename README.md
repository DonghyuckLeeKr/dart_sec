...
시계열을 현재 조회 결과가 아니라 누적 저장 데이터 기준으로 보려면 Cloudflare D1을 연결합니다. R2는 원문 PDF를 처음 내려받을 때 캐시해 DART PDF 재요청을 줄이는 용도입니다. 두 바인딩은 선택 사항이라 없으면 기존 실시간 조회 방식으로 그대로 동작합니다.

2026년 5월 기준 Cloudflare 공식 문서상 D1은 Free 플랜에서 일 500만 row read, 일 10만 row write, 총 5GB 저장까지 포함되고, R2 Standard는 월 10GB-month 저장, Class A 100만 요청, Class B 1,000만 요청까지 무료 구간이 있습니다. 팀 내부 소량 사용이면 우선 무료 구간에서 시작할 수 있습니다.

```powershell
npx wrangler d1 create dart-sec-analyze
npx wrangler r2 bucket create dart-sec-analyze-pdfs
```

생성된 D1 `database_id`를 `wrangler.toml`의 주석 처리된 `[[d1_databases]]`에 넣고, R2 버킷도 `[[r2_buckets]]` 주석을 해제해 연결합니다.

```toml
[[d1_databases]]
binding = "DART_DB"
database_name = "dart-sec-analyze"
database_id = "생성된_database_id"

[[r2_buckets]]
binding = "DART_BUCKET"
bucket_name = "dart-sec-analyze-pdfs"
```

스키마 적용:

```powershell
npx wrangler d1 migrations apply dart-sec-analyze --remote
```

이후 `/api/analyze`가 성공할 때마다 회사/공시/재무지표가 D1에 저장되고, `/api/history`가 저장된 기간을 반환합니다. 화면의 `시계열 추이`는 D1에 둘 이상의 기간이 저장되어 있으면 저장 데이터 기준으로 그립니다. `/api/pdf`는 R2가 연결된 경우 첫 다운로드 후 `pdf/{접수번호}.pdf`로 캐시합니다.
